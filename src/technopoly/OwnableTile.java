...
public class OwnableTile extends GameTile {
		...
		do {
			try {
				for(int loop = 0; loop < cloneList.size(); loop++) {
					...
					System.out.println("Please enter a bid greater than '" + highBid + "' or '0' to opt out:");
					bid = input.nextInt();
					...
					if(bid >= 0 && bid <= cloneList.get(loop).getResourceBalance()) {					
						if(bid == 0) {
							...
							System.out.println("Your bid be higher than the previous one!");
							loop--; // allow same player to bid again when loop restarts
							continue;
						} else {
				...
				System.out.println("\n*Invalid Input, restarting auction..*");
				gameManager.resetBids(gameManager.players); //resets player bids
				highBid = 49;
	...
	public void auctionTile(Player player, GameVariables gameVars, java.util.Scanner input, GameManager gameManager) {
		...
		int bid, highBid, winnerIndex;
		bid = 0;
		// allows the seller player to set the starting bid
		System.out.println(player.getPlayerName() + ", please set the starting bid:");
		highBid = input.nextInt();		
		ArrayList<Player> cloneList = gameManager.cloneList(gameManager.players);
		...
		do {
			try {
				for(int loop = 0; loop < cloneList.size(); loop++) {
					...
					// valid bidding range is between 0 and player's current resource balance
					if(bid >= 0 && bid <= cloneList.get(loop).getResourceBalance()) {					
						if(bid == 0 && cloneList.size() > 1) {
							// zero bid removes player from the bidding process
							cloneList.remove(cloneList.get(loop));
							loop--; // to compensate for the removal of a player and process next player, now at same index position as opted out player
							System.out.println("You have opted out of bidding.");
						// if bid is zero and player is the last one left in the bidding, empty it and exit loop
						} else if (bid == 0 && cloneList.size() == 1) {
							cloneList.remove(0);
							break;
						} else if (bid <= highBid) {
							System.out.println("Your bid be higher than the previous one!");
				...
				System.out.println("Invalid Input, restarting auction..");
				gameManager.resetBids(gameManager.players); //resets player bids
				input.next(); // discard non-integer input
				continue; // invalid input type, restart loop
			}
		...
		// as long as one person remains in the bidding list - auction winner
		if(cloneList.size() == 1) {
			System.out.println("\nWinning bid is " + cloneList.get(0).getPlayerBid() + " " + gameVars.getCurrency() + " from " + cloneList.get(0).getPlayerName() + "!");
			System.out.println(getTileName() + " now acquired.\n");
		...
		// no one left in the bidding list
		} else {
			System.out.println("No one wants it! Try lowering the starting bid next time!");
		}
