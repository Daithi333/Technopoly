...
public class GameManager {
	...
	/**
	 * Method to manage the setup of player objects and restart process if required
	 */
	...
	/**
	 * Method to take instantiate player objects and add to player list
	 * @param players
	 * @param numPlayers
	 * @param input
	 * @param gameVars
	...
	/**
	 * Method to check for duplicate name in list and return true if there is.
	 * @param players
	 * @param name
	 * @return boolean
	 */
	...
	/**
	 * Choice menu at beginning of each turn for each player - display holdings, manage tiles, roll dice or end game
	 * @param input
	 * @return int
	 */
	...
	/**
	 * Method allows selection of eligible tiles for the player to upgrade 
	 * @param player
	 * @param gameVars
	 * @param gameBoard
	 */
		...
		for (GameTile g : gameBoard) {
			if(g.getClass().equals(OwnableTile.class)) {
				...
				if(((OwnableTile)g).isUpgradable() && ((OwnableTile)g).getUpgradeLevel() < OwnableTile.MAX_LEVEL && player.getPlayerID() == ((OwnableTile)g).getOwnerID()) {
					...
					upgradableCount++;
					// so player doesn't go broke upgrading tiles..
					if(player.getResourceBalance() > ((OwnableTile)g).getUpgradeCost()) {
						...
						System.out.println("\n*You have insufficient resources to upgrade this tile!*");
						continue;
					}
				}
			}
	...
	/**
	 * Method allows selection of eligible tiles for the player to downgrade 
	 * @param player
	 * @param gameVars
	 * @param gameBoard
	 */
			...
			if(g.getClass().equals(OwnableTile.class)) {
				...
				if(player.getPlayerID() == ((OwnableTile)g).getOwnerID()) {
					...
					if(((OwnableTile)g).isUpgradable() == false) {
						System.out.println(((OwnableTile)g).getTileName() + " - " + ((OwnableTile)g).getFieldName());
						System.out.println("Would you like to auction this tile to the highest bidder? [Y/N]");
						auctionableCount++;
						...
						System.out.println(((OwnableTile)g).getTileName() + " cannot be auctioned as you have a Technopoly on " + ((OwnableTile)g).getFieldName() + "\n");
					}
				}
			}
	...
	public void gameOver(ArrayList<Player> players, GameVariables gameVars) {
		...
		System.out.println("******************************\n");
		System.out.println(winner.getPlayerName() + " has shown the competition how 'ruthless corporate executive' should be done!");
		System.out.println("Perhaps the defeated should reconsider their positions, maybe sign up for the Apprentice!");
		System.out.println("\n******************");
